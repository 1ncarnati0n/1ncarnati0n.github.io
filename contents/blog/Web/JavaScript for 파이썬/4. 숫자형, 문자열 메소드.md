...
`.length`는 메소드는 아니고 프로퍼티인데요. 배열의 `.length`처럼 문자열의 길이를 가져올 수 있습니다.
...
이걸 활용해서, 예를 들어 문자열이 팰린드롬(palindrome)인지 확인할 수 있는데요. `토마토`나 `기러기`처럼 거꾸로 읽어도 똑같은 단어를 팰린드롬이라고 부릅니다.
